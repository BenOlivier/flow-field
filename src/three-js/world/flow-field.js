...
export default class flowField
{
    constructor()
    {
        ...
        this.lineMats = [
            new MeshLineMaterial({ color: palette[0],
                lineWidth: randomInRange(this.params.minWidth, this.params.maxWidth) }),
            new MeshLineMaterial({ color: palette[1],
                lineWidth: randomInRange(this.params.minWidth, this.params.maxWidth) }),
            new MeshLineMaterial({ color: palette[2],
                lineWidth: randomInRange(this.params.minWidth, this.params.maxWidth) }),
            new MeshLineMaterial({ color: palette[3],
                lineWidth: randomInRange(this.params.minWidth, this.params.maxWidth) }),
            new MeshLineMaterial({ color: palette[4],
                lineWidth: randomInRange(this.params.minWidth, this.params.maxWidth) }),
        ];
    ...
    instantiateLine()
    {
        ...
        this.spawnPos.copy(this.camera.camera.position).add(this.mouseVec
            .multiplyScalar((0 - this.camera.camera.position.z ) / this.mouseVec.z));

        const randomDir = new THREE.Vector3(Math.random() - 0.5,
            Math.random() - 0.5, Math.random() - 0.5).normalize();
        ...
        for (const line of this.lines)
        {
            ...
            const noise = this.simplex.noise3D(
                ...
                line.points[line.points.length - 1].z * this.params.noiseScale +
                    this.time.elapsed * this.params.noiseSpeed) * Math.PI * 2;
